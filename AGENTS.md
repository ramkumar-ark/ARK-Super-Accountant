...
## Project status: docs/superpowers/, not .planning/

`.planning/` is archived as design-only history (captain's ruling, 2026-09-07 —
see `.planning/README.md`). Its progress tables and commit references are known
to be wrong. Current project status runs off `docs/superpowers/`, which is
gitignored — that risk has been explicitly accepted, so treat it as canon
despite being untracked and single-machine.
